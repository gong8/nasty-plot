...
function evalStatus(team: BattlePokemon[]): number {
  ...
  for (const p of team) {
    ...
    switch (p.status) {
      case "brn": score += W.STATUS_BRN; break;
      case "par": score += W.STATUS_PAR; break;
      case "slp": score += W.STATUS_SLP; break;
      case "tox": score += W.STATUS_TOX; break;
      case "psn": score += W.STATUS_PSN; break;
      case "frz": score += W.STATUS_FRZ; break;
    }
  }
  return score;
}
...
export function evaluatePosition(
  ...
  features.push({ name: "HP remaining", rawValue: myHp - oppHp, weight: W.HP_REMAINING, contribution: hpDiff });
  rawScore += hpDiff;
  ...
  features.push({ name: "Pokemon alive", rawValue: myAlive - oppAlive, weight: W.POKEMON_ALIVE, contribution: aliveDiff });
  rawScore += aliveDiff;
  ...
  features.push({ name: "Fast mons alive", rawValue: fastDiff, weight: W.FAST_POKEMON_ALIVE, contribution: fastScore });
  rawScore += fastScore;
  ...
  features.push({ name: "Screens/Tailwind", rawValue: screenDiff, weight: 1, contribution: screenDiff });
  rawScore += screenDiff;
