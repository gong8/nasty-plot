import { runBatchSimulation } from "../simulation/batch-simulator";
import type { BatchSimConfig, BatchSimProgress } from "../simulation/batch-simulator";
import type { SingleBattleResult } from "../simulation/automated-battle-manager";
import type { BattleState } from "../types";

// Mock the automated battle manager
vi.mock("../simulation/automated-battle-manager", () => ({
  runAutomatedBattle: vi.fn(),
}));

// Mock AI classes - must use class syntax for `new` to work
vi.mock("../ai/random-ai", () => ({
  RandomAI: class {
    difficulty = "random";
    chooseAction = vi.fn();
  ...
  return {
    ...
    sides: {
      p1: {
        ...
        team: [
          {
            speciesId: "garchomp", name: "Garchomp", nickname: "Garchomp",
            level: 100, types: ["Dragon", "Ground"], hp: 200, maxHp: 319,
            hpPercent: 63, status: "", fainted: false, item: "Life Orb",
            ability: "Rough Skin", isTerastallized: false, moves: [],
            stats: { hp: 319, atk: 394, def: 226, spa: 176, spd: 206, spe: 333 },
            boosts: { atk: 0, def: 0, spa: 0, spd: 0, spe: 0, accuracy: 0, evasion: 0 },
            volatiles: [],
          },
          {
            speciesId: "clefable", name: "Clefable", nickname: "Clefable",
            level: 100, types: ["Fairy"], hp: 0, maxHp: 394,
            hpPercent: 0, status: "", fainted: true, item: "Leftovers",
            ability: "Magic Guard", isTerastallized: false, moves: [],
      ...
      p2: {
        ...
        team: [
          {
            speciesId: "heatran", name: "Heatran", nickname: "Heatran",
            level: 100, types: ["Fire", "Steel"], hp: 0, maxHp: 311,
            hpPercent: 0, status: "", fainted: true, item: "Leftovers",
            ability: "Flash Fire", isTerastallized: false, moves: [],
...
function makeMockResult(
  ...
  return {
    ...
    team2Paste: "team2",
    turnActions: [],
    finalState: state ?? makeMockFinalState({ winner: winner === "draw" ? null : winner, turn: turnCount }),
  };
}
...
describe("runBatchSimulation", () => {
  ...
  it("runs the correct number of games and returns results", async () => {
    const { runAutomatedBattle } = await import("../simulation/automated-battle-manager");
