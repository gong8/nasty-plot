...
describe("validateTeam", () => {
  ...
  it("allows one Mega Stone on a team", () => {
    ...
    const result = validateTeam(team);
    const megaErrors = result.errors.filter((e) =>
      e.message.includes("Mega Evolution")
    );
  ...
  it("rejects incompatible Mega Stone", () => {
    const team = makeTeam({
      slots: [
        makeSlot("garchomp", { position: 1, item: "charizarditex" }),
      ],
    });

    const result = validateTeam(team);
    const compatErrors = result.errors.filter((e) =>
  ...
  it("allows type-based Z-Crystal with matching move type", () => {
    ...
    const result = validateTeam(team);
    const zErrors = result.errors.filter((e) =>
      e.message.includes("type move") || e.message.includes("requires")
    );
  ...
  it("rejects type-based Z-Crystal with no matching move type", () => {
    ...
    const result = validateTeam(team);
    const zErrors = result.errors.filter((e) =>
      e.message.includes("Electric-type move")
    );
  ...
  it("rejects signature Z-Crystal on wrong Pokemon", () => {
    ...
    const result = validateTeam(team);
    const zErrors = result.errors.filter((e) =>
      e.message.includes("can only be held by")
    );
  ...
  it("rejects signature Z-Crystal without required move", () => {
    ...
    const result = validateTeam(team);
    const zErrors = result.errors.filter((e) =>
      e.message.includes("requires the move")
    );
