...
export async function mergeTeams(
  ...
  return prisma.$transaction(async (tx) => {
    const mergeNotes = options?.notes
      ? options.notes
      : `Merged from ${teamB.name} (${teamBId})`;
...
export async function getLineageTree(teamId: string): Promise<LineageNode> {
  ...
  function buildNode(id: string): LineageNode | null {
    ...
    const childTeams = Array.from(allTeams.values()).filter(
      (t) => t.parentId === id,
    );
...
function diffSlot(a: TeamSlotData, b: TeamSlotData): FieldChange[] {
  ...
  function check(field: string, valA: string | number | undefined, valB: string | number | undefined) {
    if (valA !== valB) {
      changes.push({
        field,
