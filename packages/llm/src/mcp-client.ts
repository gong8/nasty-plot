...
export async function executeMcpTool(
  ...
  try {
    ...
    try {
      ...
      console.error(`${LOG_PREFIX} Tool call "${name}" failed after retry: ${retryError instanceof Error ? retryError.message : "Unknown error"}`);
  ...
  try {
    ...
    for (const uri of RESOURCE_URIS) {
      try {
        ...
        console.warn(`${LOG_PREFIX} Resource ${uri} failed: ${err instanceof Error ? err.message : "Unknown"}`);
      }
    }
