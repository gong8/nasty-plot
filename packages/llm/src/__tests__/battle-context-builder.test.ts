...
function makeSideConditions(
  overrides?: Partial<SideConditions>
): SideConditions {
  return {
    stealthRock: false,
    spikes: 0,
...
describe("buildPostBattleContext", () => {
  ...
  it("includes team names", () => {
    const result = buildPostBattleContext(
      [],
      "Stall Squad",
      "Hyper Offense",
      "p1",
...
describe("buildTurnAnalysisContext", () => {
  ...
  it("includes previous turn events when provided", () => {
    const state = makeState();
    const turnEntries = [
      makeLogEntry({ type: "move", message: "This turn move" }),
    ];
    const prevEntries = [
      makeLogEntry({ type: "move", message: "Previous turn move" }),
    ];
