...
function describePokemon(pokemon: BattlePokemon | null): string {
  ...
  if (pokemon.isTerastallized && pokemon.teraType)
    parts.push(`Tera ${pokemon.teraType}`);
...
function describeField(field: FieldState): string {
  const parts: string[] = [];
  if (field.weather)
    parts.push(`Weather: ${field.weather} (${field.weatherTurns} turns)`);
  if (field.terrain)
    parts.push(`Terrain: ${field.terrain} (${field.terrainTurns} turns)`);
  if (field.trickRoom)
    parts.push(`Trick Room (${field.trickRoom} turns)`);
...
export function buildPostBattleContext(
  ...
  const winnerName =
    winner === "p1" ? team1Name : winner === "p2" ? team2Name : "tie";
