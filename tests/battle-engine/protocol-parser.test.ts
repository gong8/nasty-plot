...
describe("protocol-parser", () => {
    ...
    it("parses a team preview request", () => {
      const request = JSON.stringify({
        ...
        side: {
          name: "Player",
          id: "p1",
          pokemon: [{ details: "Garchomp, L100", condition: "319/319", moves: [] }],
        },
      })
  ...
  describe("processLine - additional protocol messages", () => {
    ...
    it("returns info log for |-ohko| messages", () => {
      const state = makeState()
      const entry = processLine(state, "|-ohko")
    ...
    it("returns info log for |-hitcount| messages", () => {
      const state = makeState()
      const entry = processLine(state, "|-hitcount|p2a: Heatran|3")
      expect(entry).toEqual(
        expect.objectContaining({
          type: "info",
  ...
  describe("updateSideFromRequest", () => {
    ...
    it("preserves maxHp when fainted pokemon has 0 maxHp from condition", () => {
      const state = makeState()
      // First set up a pokemon with actual HP
      updateSideFromRequest(state, "p1", {
        name: "Player",
        id: "p1",
        pokemon: [
      ...
      // Now update with fainted condition (0 fnt gives maxHp=0)
      updateSideFromRequest(state, "p1", {
        name: "Player",
        id: "p1",
        pokemon: [
      ...
      // maxHp should be preserved from earlier data
      expect(state.sides.p1.team[0].maxHp).toBe(319)
      expect(state.sides.p1.team[0].hp).toBe(0)
      expect(state.sides.p1.team[0].fainted).toBe(true)
    })
  })

  describe("processLine - findPokemon searches team when not in active", () => {
    it("finds pokemon in team when not active (e.g. benched pokemon getting healed)", () => {
      const state = makeState()
      // Switch in Garchomp as active
      processLine(state, "|switch|p1a: Garchomp|Garchomp, L100|319/319")
      // Also add Heatran to team via another switch then switch back
      processLine(state, "|switch|p1a: Heatran|Heatran, L100|200/311")
      processLine(state, "|switch|p1a: Garchomp|Garchomp, L100|319/319")

      // Now heal Heatran (who is on the bench, not active)
      const entry = processLine(state, "|-heal|p1a: Heatran|250/311")

      // Heatran should be found in the team and updated
      const heatran = state.sides.p1.team.find((p) => p.name === "Heatran")
      expect(heatran).toBeDefined()
      expect(heatran!.hp).toBe(250)
      expect(entry).not.toBeNull()
    })
  })

  describe("processLine - additional sideend cases", () => {
    it("parses |-sideend| for Spikes", () => {
      const state = makeState()
      processLine(state, "|-sidestart|p1: Player|move: Spikes")
      processLine(state, "|-sidestart|p1: Player|move: Spikes")
      expect(state.sides.p1.sideConditions.spikes).toBe(2)

      processLine(state, "|-sideend|p1: Player|move: Spikes")
      expect(state.sides.p1.sideConditions.spikes).toBe(0)
    })

    it("parses |-sideend| for Sticky Web", () => {
      const state = makeState()
      processLine(state, "|-sidestart|p2: Opponent|move: Sticky Web")
      expect(state.sides.p2.sideConditions.stickyWeb).toBe(true)

      processLine(state, "|-sideend|p2: Opponent|move: Sticky Web")
      expect(state.sides.p2.sideConditions.stickyWeb).toBe(false)
    })
  })

  describe("parseRequestForSlot", () => {
    it("parses a normal turn request for slot 0", () => {
      const request = JSON.stringify({
        active: [
          {
            moves: [
      ...
      const result = parseRequestForSlot(request, 0)
      expect(result.actions).not.toBeNull()
      expect(result.actions!.moves).toHaveLength(1)
      expect(result.actions!.moves[0].name).toBe("Earthquake")
      expect(result.actions!.canTera).toBe(true)
      expect(result.actions!.activeSlot).toBe(0)
    })

    it("parses a normal turn request for slot 1", () => {
      const request = JSON.stringify({
        active: [
          {
            moves: [{ move: "Earthquake", id: "earthquake", pp: 16, maxpp: 16 }],
          },
          {
        ...
        side: {
          ...
          id: "p1",
          pokemon: [
            { details: "Garchomp, L100", condition: "319/319", moves: ["earthquake"] },
            { details: "Heatran, L100", condition: "311/311", moves: ["flamethrower"] },
          ],
        },
      })

      const result = parseRequestForSlot(request, 1)
      expect(result.actions).not.toBeNull()
      expect(result.actions!.moves).toHaveLength(1)
      expect(result.actions!.moves[0].name).toBe("Flamethrower")
      expect(result.actions!.canTera).toBe(true)
      expect(result.actions!.activeSlot).toBe(1)
    })

    it("returns null actions for wait request", () => {
      const request = JSON.stringify({ wait: true })
      const result = parseRequestForSlot(request, 0)
      expect(result.wait).toBe(true)
      expect(result.actions).toBeNull()
    ...
    it("returns teamPreview for team preview request", () => {
      const request = JSON.stringify({
        teamPreview: true,
        side: {
          name: "Player",
    ...
    it("handles forceSwitch array for doubles", () => {
      const request = JSON.stringify({
        ...
        side: {
          ...
          id: "p1",
          pokemon: [
            { details: "Garchomp, L100", condition: "0 fnt", moves: [] },
            { details: "Heatran, L100", condition: "311/311", moves: [] },
            { details: "Clefable, L100", condition: "394/394", moves: [] },
          ],
        },
      })

      // Slot 0 must switch
      const result0 = parseRequestForSlot(request, 0)
      expect(result0.actions).not.toBeNull()
      expect(result0.actions!.forceSwitch).toBe(true)

      // Slot 1 does not need to switch
      const result1 = parseRequestForSlot(request, 1)
      expect(result1.actions).toBeNull()
    })

    it("returns null when active slot does not exist", () => {
      const request = JSON.stringify({
        active: [{ moves: [{ move: "Earthquake", id: "earthquake", pp: 16, maxpp: 16 }] }],
        side: {
          name: "Player",
          id: "p1",
          pokemon: [{ details: "Garchomp, L100", condition: "319/319", moves: [] }],
      ...
      // Slot 1 doesn't exist in active array
      const result = parseRequestForSlot(request, 1)
      expect(result.actions).toBeNull()
    })
  })

  describe("processLine - ohko and hitcount produce log entries", () => {
    it("parses |-ohko| as info log entry", () => {
      const state = makeState()
      const entry = processLine(state, "|-ohko")
      expect(entry).not.toBeNull()
      expect(entry!.type).toBe("info")
      expect(entry!.message).toContain("one-hit KO")
    })

    it("parses |-hitcount| with count", () => {
      const state = makeState()
      const entry = processLine(state, "|-hitcount|p2a: Heatran|3")
      expect(entry).not.toBeNull()
      expect(entry!.type).toBe("info")
  ...
  describe("processLine - weather edge cases", () => {
    it("parses special weather types: DesolateLand, PrimordialSea, DeltaStream", () => {
      ...
      processLine(state, "|-weather|DesolateLand")
      expect(state.field.weather).toBe("Desolate Land")

      processLine(state, "|-weather|PrimordialSea")
      expect(state.field.weather).toBe("Primordial Sea")

      processLine(state, "|-weather|DeltaStream")
      expect(state.field.weather).toBe("Delta Stream")
    })

    it("handles unmapped weather as-is", () => {
      const state = makeState()
      processLine(state, "|-weather|SomeNewWeather")
      expect(state.field.weather).toBe("SomeNewWeather")
    })
  })
})
