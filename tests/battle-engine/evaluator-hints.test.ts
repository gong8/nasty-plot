...
describe("evaluatePosition", () => {
  ...
  it("inverts speed advantage under Trick Room", () => {
    ...
    normalState.sides.p1.active = [
      makePokemon({ stats: { hp: 357, atk: 394, def: 226, spa: 196, spd: 206, spe: 333 } }),
    ]
...
describe("generateHints", () => {
  it("returns rankedMoves sorted by score descending", () => {
    ...
    for (let i = 1; i < result.rankedMoves.length; i++) {
      expect(result.rankedMoves[i - 1].score).toBeGreaterThanOrEqual(result.rankedMoves[i].score)
    }
  })
  ...
  it("worse moves have appropriate classifications (not 'best')", () => {
    ...
    if (result.rankedMoves.length > 1) {
      ...
      if (lastMove.score < result.rankedMoves[0].score) {
        const validClassifications = ["best", "good", "neutral", "inaccuracy", "mistake", "blunder"]
  ...
  it("scores Will-O-Wisp and Thunder Wave status moves", () => {
    const state = makeState()
    const actions = makeActions({
      moves: [
        {
        ...
        },
        {
          ...
          category: "Status",
          accuracy: true,
          description: "The target drowsily falls asleep next turn.",
        },
      ],
      switches: [],
    })
    ...
    const wowHint = result.rankedMoves.find((m) => m.name === "Will-O-Wisp")
    expect(wowHint).toBeDefined()
    expect(wowHint!.score).toBeGreaterThan(0)

    const twaveHint = result.rankedMoves.find((m) => m.name === "Thunder Wave")
    expect(twaveHint).toBeDefined()
    expect(twaveHint!.score).toBeGreaterThan(0)

    const spowderHint = result.rankedMoves.find((m) => m.name === "Sleep Powder")
    expect(spowderHint).toBeDefined()
    expect(spowderHint!.score).toBeGreaterThan(0)

    const yawnHint = result.rankedMoves.find((m) => m.name === "Yawn")
    expect(yawnHint).toBeDefined()
    expect(yawnHint!.score).toBeGreaterThan(0)
  })

  it("switch scoring rewards defensive/offensive coverage against opponent", () => {
    const state = makeState()
    ...
    state.sides.p2.team = [state.sides.p2.active[0]!]
    // Switch target is Ground type (SE against Fire/Steel)
    ...
    const switchHint = result.rankedMoves.find((m) => m.action.type === "switch")
    expect(switchHint).toBeDefined()
    // Switch hint should produce a score and explanation
    expect(typeof switchHint!.score).toBe("number")
    expect(switchHint!.explanation.length).toBeGreaterThan(0)
  })

  it("shows partial KO chance when max damage KOs but min does not (lines 94-96)", () => {
    const state = makeState()
    // Garchomp EQ vs Iron Valiant: damage range 148-175
    ...
    // Should display a partial KO chance explanation (e.g. "XX% chance to KO")
    expect(eqHint!.explanation).toContain("chance to KO")
  })

  it("falls back to type effectiveness when calc throws (lines 115-118)", () => {
    const state = makeState()
    ...
    state.sides.p1.team = [state.sides.p1.active[0]!]
    const actions = makeActions({
      moves: [
        {
          name: "Earthquake",
