...
describe("GET /api/sample-teams", () => {
  ...
  it("filters by formatId", async () => {
    ...
    expect(response.status).toBe(200);
    expect(listSampleTeams).toHaveBeenCalledWith(
      expect.objectContaining({ formatId: "gen9ou" }),
    );
  });
  ...
  it("filters by archetype", async () => {
    ...
    expect(response.status).toBe(200);
    expect(listSampleTeams).toHaveBeenCalledWith(
      expect.objectContaining({ archetype: "Rain" }),
    );
  });
  ...
  it("filters by search query", async () => {
    ...
    expect(response.status).toBe(200);
    expect(listSampleTeams).toHaveBeenCalledWith(
      expect.objectContaining({ search: "pelipper" }),
    );
  });
});
...
describe("DELETE /api/sample-teams/[id]", () => {
  ...
  it("returns 500 when delete fails", async () => {
    (deleteSampleTeam as ReturnType<typeof vi.fn>).mockRejectedValue(
