...
function makeDbSlot(overrides?: Record<string, unknown>) {
  return {
    ...
    move3: null,
    move4: null,
    evHp: 0, evAtk: 252, evDef: 0, evSpA: 0, evSpD: 0, evSpe: 252,
...
describe("compareTeams", () => {
  ...
  it("detects EV spread changes", () => {
    ...
    expect(diff.changed).toHaveLength(1);
    const evChanges = diff.changed[0].changes.filter((c) =>
      c.field.startsWith("evs."),
    );
    ...
    expect(fieldMap["evs.hp"]).toEqual(
      expect.objectContaining({ before: 0, after: 252 }),
    );
    expect(fieldMap["evs.atk"]).toEqual(
      expect.objectContaining({ before: 252, after: 0 }),
    );
  ...
  it("detects move changes per index", () => {
    ...
    expect(diff.changed).toHaveLength(1);
    const moveChanges = diff.changed[0].changes.filter((c) =>
      c.field.startsWith("moves["),
    );
  ...
  it("handles null fields (teraType set vs unset)", () => {
    ...
    expect(diff.changed).toHaveLength(1);
    const teraChange = diff.changed[0].changes.find(
      (c) => c.field === "teraType",
    );
...
describe("forkTeam", () => {
  ...
  it("copies all slots to the forked team", async () => {
    const sourceSlots = [
      ...
      makeDbSlot({
        id: 2, position: 2, pokemonId: "heatran", teamId: "source-1",
        ability: "Flash Fire", item: "Air Balloon", nature: "Timid",
        move1: "Magma Storm", move2: "Earth Power", move3: null, move4: null,
        evHp: 0, evAtk: 0, evDef: 0, evSpA: 252, evSpD: 4, evSpe: 252,
      }),
    ];
    const sourceDb = makeDbTeam({
      id: "source-1",
  ...
  it("applies modifySlots option", async () => {
    ...
    const forkedDb = makeDbTeam({
      ...
      parentId: "source-1",
      slots: [
        makeDbSlot({ teamId: "fork-1", item: "Choice Band" }),
      ],
    });
...
describe("mergeTeams", () => {
  ...
  it("creates merged team with correct slots per decisions", async () => {
    const slotGarchomp = makeDbSlot({
      position: 1, pokemonId: "garchomp", teamId: "team-a",
    });
    const slotHeatran = makeDbSlot({
      id: 2, position: 2, pokemonId: "heatran", teamId: "team-a",
      ability: "Flash Fire", item: "Air Balloon", nature: "Timid",
      move1: "Magma Storm", move2: "Earth Power", move3: null, move4: null,
      evHp: 0, evAtk: 0, evDef: 0, evSpA: 252, evSpD: 4, evSpe: 252,
    });
    const slotLandorus = makeDbSlot({
      id: 3, position: 2, pokemonId: "landorus", teamId: "team-b",
      ability: "Intimidate", item: "Choice Scarf", nature: "Jolly",
      move1: "Earthquake", move2: "U-turn", move3: null, move4: null,
    });
    ...
    const dbTeamB = makeDbTeam({
      ...
      name: "Team B",
      slots: [
        makeDbSlot({ position: 1, pokemonId: "garchomp", teamId: "team-b" }),
        slotLandorus,
      ],
    });
    ...
    mockTeamFindUnique
      .mockResolvedValueOnce(dbTeamA)
      .mockResolvedValueOnce(dbTeamB);
    ...
    const txTeamFindUnique = vi.fn().mockResolvedValue(
      makeDbTeam({
        ...
        slots: [
          ...
          makeDbSlot({ position: 2, pokemonId: "landorus", teamId: "merge-1",
            ability: "Intimidate", item: "Choice Scarf", nature: "Jolly",
            move1: "Earthquake", move2: "U-turn", move3: null, move4: null }),
        ],
      }),
    ...
    const decisions: MergeDecision[] = [
      { pokemonId: "landorus", source: "teamB" },
    ];
