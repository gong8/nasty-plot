import { getToolLabel, isWriteTool } from "@nasty-plot/llm"
...
describe("getToolLabel", () => {
  ...
  it("returns label for known MCP-prefixed tool name", () => {
    expect(getToolLabel(`${MCP_PREFIX}get_pokemon`)).toBe("Looking up Pokemon data")
  })
  ...
  it("returns label for analysis tools", () => {
    expect(getToolLabel("analyze_team_coverage")).toBe("Analyzing team coverage")
    expect(getToolLabel("find_team_weaknesses")).toBe("Finding team weaknesses")
  ...
  it("returns label for team-crud tools", () => {
    ...
    expect(getToolLabel("add_pokemon_to_team")).toBe("Adding Pokemon to team")
    expect(getToolLabel("update_pokemon_set")).toBe("Updating Pokemon set")
    expect(getToolLabel("remove_pokemon_from_team")).toBe("Removing Pokemon from team")
  })

  it("returns label for meta-recs tools", () => {
    expect(getToolLabel("get_meta_trends")).toBe("Checking meta trends")
    expect(getToolLabel("get_format_viability")).toBe("Checking format viability")
    expect(getToolLabel("get_usage_stats")).toBe("Fetching usage stats")
    expect(getToolLabel("suggest_teammates")).toBe("Finding teammate suggestions")
  ...
  it("strips MCP prefix before fallback", () => {
    expect(getToolLabel(`${MCP_PREFIX}unknown_tool`)).toBe("Running unknown_tool")
  })
})
