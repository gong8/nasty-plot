...
describe("StreamParser", () => {
  ...
  describe("process", () => {
    ...
    it("preserves text before and after plan tags", () => {
      const result = parser.process(
        "Before<plan><step>Do thing</step></plan>After"
      );
    ...
    it("parses step_update self-closing tags", () => {
      const result = parser.process(
        'Text<step_update index="0" status="active"/>More'
      );
    ...
    it("parses step_update with complete status", () => {
      const result = parser.process(
        '<step_update index="1" status="complete"/>'
      );
      expect(result.cleanContent).toBe("");
      expect(result.events).toHaveLength(1);
      expect(result.events[0]).toEqual({
    ...
    it("parses step_update with skipped status", () => {
      const result = parser.process(
        '<step_update index="2" status="skipped"/>'
      );
      expect(result.events[0]).toEqual({
        type: "plan_step_update",
        stepIndex: 2,
    ...
    it("handles interleaved text and step_updates", () => {
      const result = parser.process(
        'Some text<step_update index="0" status="complete"/> more text'
      );
