...
describe("calculateStat", () => {
  ...
  it("calculates HP correctly for Garchomp at Lv100, 31 IV, 0 EV", () => {
    expect(calculateStat("hp", 108, PERFECT_IV, 0, DEFAULT_LEVEL, "Hardy")).toBe(357)
  })
  ...
  it("calculates HP correctly for Garchomp at Lv100, 31 IV, 252 EV", () => {
    expect(calculateStat("hp", 108, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Hardy")).toBe(420)
  })

  // Shedinja special case: HP is always 1 regardless of IVs/EVs
  it("returns 1 for Shedinja HP (base 1)", () => {
    expect(calculateStat("hp", 1, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Hardy")).toBe(1)
  })

  it("returns 1 for Shedinja HP at any level", () => {
    expect(calculateStat("hp", 1, PERFECT_IV, 0, 50, "Adamant")).toBe(1)
  })
  ...
  it("calculates Attack with positive nature (Adamant)", () => {
    expect(calculateStat("atk", 130, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Adamant")).toBe(394)
  })
  ...
  it("calculates Attack with negative nature (Modest)", () => {
    expect(calculateStat("atk", 130, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Modest")).toBe(323)
  })

  // Neutral nature for Atk: Hardy has no plus/minus
  // floor((354 + 5) * 1.0) = 359
  it("calculates Attack with neutral nature (Hardy)", () => {
    expect(calculateStat("atk", 130, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Hardy")).toBe(359)
  })
  ...
  it("calculates stats correctly at Level 50", () => {
    expect(calculateStat("atk", 130, PERFECT_IV, MAX_SINGLE_EV, 50, "Adamant")).toBe(200)
  })
  ...
  it("calculates stats at Level 1", () => {
    expect(calculateStat("atk", 130, PERFECT_IV, 0, 1, "Hardy")).toBe(7)
  })
  ...
  it("calculates stats with 0 IV and 0 EV", () => {
    expect(calculateStat("atk", 130, 0, 0, DEFAULT_LEVEL, "Hardy")).toBe(265)
  })
  ...
  it("calculates HP correctly at Level 50", () => {
    expect(calculateStat("hp", 108, PERFECT_IV, MAX_SINGLE_EV, 50, "Hardy")).toBe(215)
  })
  ...
  it("applies positive nature to SpA for Modest", () => {
    expect(calculateStat("spa", 80, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Modest")).toBe(284)
  })
  ...
  it("applies neutral modifier for unaffected stat", () => {
    expect(calculateStat("def", 95, PERFECT_IV, 0, DEFAULT_LEVEL, "Adamant")).toBe(226)
  })
  ...
  it("calculates Speed with Jolly nature", () => {
    expect(calculateStat("spe", 102, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Jolly")).toBe(333)
  })
  ...
  it("calculates Pikachu HP correctly", () => {
    expect(calculateStat("hp", 35, PERFECT_IV, 0, DEFAULT_LEVEL, "Hardy")).toBe(211)
  })

  // Blissey HP: base 255, 31 IV, 252 EV
  // HP: floor((2*255+31+63)*100/100) + 100 + 10 = 604 + 110 = 714
  it("calculates Blissey HP (highest base HP)", () => {
    expect(calculateStat("hp", 255, PERFECT_IV, MAX_SINGLE_EV, DEFAULT_LEVEL, "Hardy")).toBe(714)
  })
  ...
  it("handles EV values not divisible by 4 via floor", () => {
    expect(calculateStat("atk", 100, PERFECT_IV, 7, DEFAULT_LEVEL, "Hardy")).toBe(237)
  })
  ...
  it("handles 0 base stat for HP", () => {
    expect(calculateStat("hp", 0, PERFECT_IV, 0, DEFAULT_LEVEL, "Hardy")).toBe(141)
  })

  // Edge: 0 base stat for non-HP
  // core = 31, result = floor((31+5)*1.0) = 36
  it("handles 0 base stat for non-HP", () => {
    expect(calculateStat("atk", 0, PERFECT_IV, 0, DEFAULT_LEVEL, "Hardy")).toBe(36)
  })
  ...
  it.each(["Bashful", "Docile", "Hardy", "Quirky", "Serious"] as NatureName[])(
    ...
    (nature) => {
      expect(calculateStat("atk", 100, PERFECT_IV, 0, DEFAULT_LEVEL, nature)).toBe(
        calculateStat("atk", 100, PERFECT_IV, 0, DEFAULT_LEVEL, "Hardy"),
      )
    },
  )
})
...
describe("calculateAllStats", () => {
  ...
  it("calculates all stats for a standard Garchomp spread", () => {
    const evs = makeEvs({ atk: MAX_SINGLE_EV, spd: 4, spe: MAX_SINGLE_EV })
    const ivs = makeIvs()
    const result = calculateAllStats(garchompBase, ivs, evs, DEFAULT_LEVEL, "Jolly")
  ...
  it("returns all six stat keys", () => {
    const evs = makeEvs()
    const ivs = makeIvs()
    const result = calculateAllStats(garchompBase, ivs, evs, DEFAULT_LEVEL, "Hardy")
    expect(Object.keys(result).sort()).toEqual(["atk", "def", "hp", "spa", "spd", "spe"])
  })

  it("calculates all stats at Level 50", () => {
    const evs = makeEvs({ hp: MAX_SINGLE_EV, atk: MAX_SINGLE_EV, spe: 4 })
    const ivs = makeIvs()
...
describe("getTotalEvs", () => {
  ...
  it("sums all EV values", () => {
    expect(getTotalEvs(makeEvs({ hp: MAX_SINGLE_EV, atk: MAX_SINGLE_EV, spe: 4 }))).toBe(508)
  })
...
describe("validateEvs", () => {
  it("accepts a valid competitive spread", () => {
    const evs = makeEvs({ hp: MAX_SINGLE_EV, def: 4, spe: MAX_SINGLE_EV })
  ...
  it("accepts exactly 510 total", () => {
    const evs = makeEvs({ hp: MAX_SINGLE_EV, atk: MAX_SINGLE_EV, def: 4, spe: 0, spa: 0, spd: 2 })
  ...
  it("accepts 510 total with all divisible by 4", () => {
    ...
    const evs = makeEvs({ hp: MAX_SINGLE_EV, atk: MAX_SINGLE_EV, def: 4 })
    expect(validateEvs(evs)).toEqual({ valid: true })
  })
  ...
  it("accepts exactly 252 for a single stat", () => {
    const evs = makeEvs({ spa: MAX_SINGLE_EV })
    expect(validateEvs(evs)).toEqual({ valid: true })
  })
