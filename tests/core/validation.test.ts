...
describe("validateTeam", () => {
  ...
  describe("individual EV values", () => {
    ...
    it("allows EV of exactly 0", () => {
      const team = makeTeam([makeSlot({ position: 1, evs: makeEvs() })])
      const evStatErrors = validateTeam(team).errors.filter(
        (e) => e.field.includes("evs.") && e.message.includes("must be between"),
      )
    ...
    it("reports errors for multiple invalid EVs on the same slot", () => {
      ...
      const result = validateTeam(team)
      const evStatErrors = result.errors.filter((e) => e.message.includes("must be between"))
  ...
  describe("pokemon selection", () => {
    it("flags a slot with empty pokemonId", () => {
      const team = makeTeam([makeSlot({ position: 1, pokemonId: "" })])
      const result = validateTeam(team)
      expect(result.valid).toBe(false)
