...
- **"Only increase maxHp" guard in protocol handlers**: Rather than trying to detect percentage vs absolute HP format (which would be fragile), the fix uses a simple invariant: protocol lines can only *increase* `maxHp`, never decrease it. This works because:
