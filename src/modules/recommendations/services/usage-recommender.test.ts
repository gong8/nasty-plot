...
describe("getUsageBasedRecommendations", () => {
  ...
  describe("score aggregation", () => {
    it("aggregates correlation scores across multiple team members", async () => {
      ...
      // Heatran appears in 2 correlations: avg = (30 + 25) / 2 = 27.5
      // score = min(100, round(27.5 * 2)) = 55
    ...
    it("caps score at 100", async () => {
      ...
      // avg = (80 + 90) / 2 = 85; score = min(100, round(85*2)) = 100
    ...
    it("calculates score as avgCorrelation * 2 rounded", async () => {
      ...
      expect(rec).toBeDefined();
      // avg = 15/1 = 15; score = min(100, round(15*2)) = 30
  ...
  describe("reasons", () => {
    ...
    it("uses pokemonAId as fallback name when species doesn't exist", async () => {
      ...
      if (result.length > 0) {
        // The reason should still reference the partner even if it doesn't exist in Dex
