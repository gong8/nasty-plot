import { DEFAULT_EVS, DEFAULT_IVS, NATURE_DATA } from "@/shared/constants";
import type { NatureName, StatsTable, TeamSlotData } from "@/shared/types";
...
export function parseShowdownPaste(paste: string): Partial<TeamSlotData>[] {
  ...
  return blocks.map((block, i) => parseOneSlot(block.trim(), i + 1)).filter(Boolean) as Partial<TeamSlotData>[];
}
