import { parseShowdownPaste, serializeShowdownPaste, toId } from "./showdown-paste";
import { DEFAULT_EVS, DEFAULT_IVS } from "@/shared/constants";
import type { NatureName, TeamSlotData, StatsTable } from "@/shared/types";
...
function makeSlot(overrides: Partial<TeamSlotData> = {}): TeamSlotData {
  return {
    position: 1,
    pokemonId: "garchomp",
    species: { id: "garchomp", name: "Garchomp", num: 445, types: ["Dragon", "Ground"], baseStats: { hp: 108, atk: 130, def: 95, spa: 80, spd: 85, spe: 102 }, abilities: { "0": "Sand Veil" }, weightkg: 95 },
    ability: "Rough Skin",
    item: "Rocky Helmet",
...
describe("serializeShowdownPaste", () => {
  ...
  it("uses species name when available", () => {
    const slot = makeSlot({
      species: { id: "greattusk", name: "Great Tusk", num: 984, types: ["Ground", "Fighting"], baseStats: { hp: 115, atk: 131, def: 131, spa: 53, spd: 53, spe: 87 }, abilities: { "0": "Protosynthesis" }, weightkg: 320 },
    });
    const output = serializeShowdownPaste([slot]);
  ...
  it("serializes multiple slots separated by double newlines", () => {
    ...
    const slot2 = makeSlot({
      position: 2,
      pokemonId: "pikachu",
      species: { id: "pikachu", name: "Pikachu", num: 25, types: ["Electric"], baseStats: { hp: 35, atk: 55, def: 40, spa: 50, spd: 50, spe: 90 }, abilities: { "0": "Static" }, weightkg: 6 },
      item: "Light Ball",
      ability: "Static",
