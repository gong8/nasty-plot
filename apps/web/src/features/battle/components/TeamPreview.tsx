...
export function TeamPreview({
  ...
  const handleDoublesSubmit = () => {
    ...
    const remaining = Array.from(selectedBring).filter(
      (i) => !selectedLeads.includes(i)
    );
    const order = [
      selectedLeads[0] + 1,
      selectedLeads[1] + 1,
  ...
  if (doublesPhase === "pick") {
    return (
      <div className={cn("space-y-6", className)}>
        ...
        <div className="flex justify-center">
          <Button
            onClick={handleDoublesNextPhase}
            disabled={selectedBring.size !== 4}
            size="lg"
          >
            Next: Choose Leads
          </Button>
