...
function getSuggestion(
      ...
      return {
        ...
        message: (
          <>
            ...
            <span className="font-medium capitalize">{weakness}</span>{" "}
    ...
    return {
      ...
      message: (
        <>
          ...
          <ConceptTooltip term="hazard removal">remove hazards</ConceptTooltip>
          ? A{" "}
  ...
  if (filledSlotCount < 3) {
    return {
      ...
      message: (
        <>
          Focus on building a strong offensive or defensive core first.
        </>
      ),
    };
  }
  ...
  return {
    ...
    message: (
      <>
        Pick a Pokemon that complements your team. The recommendations below
        account for your current{" "}
        <ConceptTooltip term="coverage">coverage</ConceptTooltip>.
      </>
    ),
  };
...
export function RoleSuggestionBanner({
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-3 rounded-lg border bg-muted/50 px-4 py-3"
      )}
    >
      <Icon
        className={cn(
