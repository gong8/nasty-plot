...
export async function POST(req: NextRequest) {
  try {
    const [body, error] = await validateBody(req, chatMessageSchema)
    ...
    const { effectiveTeamId, effectiveFormatId } = resolveContextIds({
      ...
      teamId,
      formatId,
      pageTeamId: (context as PageContextData | undefined)?.teamId,
      pageFormatId: (context as PageContextData | undefined)?.formatId,
    })
    ...
    const stream = await streamChat({
      ...
      signal: req.signal,
      context: context as PageContextData | undefined,
